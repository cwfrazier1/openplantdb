...
| [`schema/table.sql`](schema/table.sql) | PostgreSQL `CREATE TABLE` (the upsert bundle `schema/plants.sql` is generated by `build.py`) |
...
```bash
python scripts/load_postgres.py "$YOUR_DSN"   # loads straight from data/plants.json
```

Prefer the pure-SQL path with no Python deps? The `schema/plants.sql` upsert
bundle is **generated** (it's large enough that git won't track it), so build it
first:

```bash
python scripts/build.py                        # (re)generates schema/plants.sql
